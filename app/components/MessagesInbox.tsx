...
export default function MessagesInbox({ currentUserId }: Props) {
  ...
  const ConversationList = (
    <div className="w-full sm:w-1/3 sm:border-r border-gray-200 overflow-y-auto">
      {conversations.map((c) => {
        const isSelected =
  ...
  const ChatPane = selected ? (
    <div className="flex-1 flex flex-col">
      <div className="p-4 flex-1 overflow-y-auto">
        <button
          ...
          className="sm:hidden mb-3 text-sm text-blue-600"
        >
    ...
    <p className="hidden sm:block text-sm text-gray-400 p-6">Select a conversation to view messages.</p>
  );

  return (
    <div className="flex border border-gray-200 rounded-lg overflow-hidden" style={{ minHeight: "500px" }}>
      <div className={mobileShowChat ? "hidden sm:block sm:w-1/3" : "w-full sm:w-1/3"}>
        {ConversationList}
      </div>
      <div className={mobileShowChat ? "flex-1 flex flex-col" : "hidden sm:flex sm:flex-1 sm:flex-col"}>
        {ChatPane}
      </div>
    </div>
  );
